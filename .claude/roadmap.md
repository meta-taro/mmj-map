...
- [ ] ~~永続レイアウト~~ — **この項目は言葉だけで、中身が決まっていません。**
      おそらく「ページを移っても Map を作り直さず、位置・ズーム・WebGL コンテキストを
      保ったまま周りの UI だけ差し替える」形を指しています。
      ただしそれを部品側で持つと**地図の状態を抱え込む**ことになり、
      「ここから厚くしない」約束（PRD §3・`docs/elements/README.md`）と衝突します。
      **提案は「消す」**です。要るのは機能ではなく
      「`<mmj-map>` を DOM から外さない」という使い方の説明 1 節。
      止めるなら、何を指していたのかを教えてください（2026-09-17）
...
- [ ] ベース地図の POI を描く — 駅しか描いていないことが分かった
