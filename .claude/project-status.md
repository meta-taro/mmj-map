...
- **現在フェーズ**: S1 は完了条件を満たした（**人の目視判定待ち**）。S2（UI 部品）は
  Map / Marker / Popup / Cluster まで。残りは永続レイアウト
- **最終更新**: 2026-09-17
...
5. フィルタが実際に何件拾うかを数える道具（いまはタイルを開いて手で数えている）。
   **やるなら提案を先に出す**
6. S2 の残り＝**永続レイアウト**（地図の状態を保ったまま周りだけ差し替わる形）。
   これは「部品を厚くしない」約束（PRD §3）と正面からぶつかる可能性があるので、
   **実装より先に提案を出す**
...
| `packages/elements` | 2 | 28 |
| **合計** | **15** | **165 すべて通過**（2026-09-17 実行） |
