...
- **現在フェーズ**: S1 は完了条件を満たした（**人の目視判定待ち**）。S2（UI 部品）は
  Map / Marker / Popup / Cluster まで。残りの「永続レイアウト」は**中身が未定義**（下記）
- **最終更新**: 2026-09-17
...
- **全国の切り出しから、太平洋を落とした（D-013・2026-09-17）** — `--bbox` 1 個ではなく
  **8 つの四角**（`tools/tiles/regions/japan.geojson`）で切る。実測で
  **3.6 GB → 2.8 GB（-22%）/ タイル件数 -53%**。**遠方の島は 1 つも落としていない**
  （35 点の被覆をテストで止めている）。**本番の切り出しはまだ流していない**
...
8. 「永続レイアウト」が何を指していたのかを確かめる。
   **roadmap に言葉だけが残っていて、中身が決まっていません**（提案は「消す」）
...
**2026-09-17 追記**: 上は `--bbox` で切ったときの値です。**8 つの四角へ変えました**（D-013）。
dry-run の実測で **タイル件数 816,723 / 2.8 GB**（-53% / -22%）になります。
**遠方の島は落としていません。****本番の切り出しはまだ流していないので、
`dist/tiles/japan.pmtiles` は 3.6 GB の bbox 版のままです。**
