...
### SNS に貼られたときの面を持った（`484cde2`）

トップ頁を X や Slack に貼っても**絵が出ていなかった**。`og:title` と
`og:description` はあったが `og:image` が無く、カードは文字だけだった。
**この製品は見た目が中身**なのに、貼られた先で見た目が 1 ピクセルも出ていなかった。
依頼は別セッションから（X と Zenn で紹介するため）。

**画像を作らず、頁を撮っている。**`apps/demo/og.html` という版下を置き、

    pnpm serve
    pnpm shot -- "http://localhost:8787/og.html" apps/demo/og.png --size=1200x630

で撮る。画像編集ソフトで作ると**直したいときに作った人しか直せない**。
地図は本物（大阪 z12.4・`demo.pmtiles`）なので、`© OpenStreetMap contributors` も
本物が出る。

`pnpm shot` に `--size=` を足した。**ブラウザの外から窓を 1200x630 にする手は
効かなかった**（拡張で resize しても `innerWidth` は 1359 のまま、`outerWidth` は 0）。
CDP の `Emulation.setDeviceMetricsOverride` で、撮る側が寸法を持つ形にしている。
読めない寸法は落とす（黙って既定で撮ると、違う大きさの絵がそのまま出回る）。

**撮って初めて分かったこと**: 帰属表示と縮尺が MapLibre 既定の**白い箱**のまま
暗いカードに乗り、貼り間違えた紙のように浮いていた。版下の中だけで暗い面へ合わせた
（**消していない・薄くしていない**。背景 `#14161a` に文字 `#d8dce1` で 12:1）。
**公開デモの各頁も同じ見え方をする**が、地図そのものの見た目は `DESIGN.md` の領域
なので製品側には手を入れていない。**人の判断待ち**。

**公開先で確認済み**（2026-09-25）:

| 見たもの | 結果 |
|---|---|
| `curl` でトップの meta | og / twitter 12 行が取れる |
| `og.png` | `200` / `image/png` / `842,107 B` |
| PNG ヘッダ | `1200x630`・commit 済みファイルとバイト一致 |
| 公開先トップを撮り直し | 地図が出る・通信失敗 0 件・console 0 件 |

入れたのは**トップ頁のみ**。他の 9 枚はトップから辿る先なので触っていない。

### 保留にしたもの — `npx @mmj-map/tiles extract` から go-pmtiles を外す件

別セッションから依頼があり、**「オーナーへの確認は、この返信をもって済んだものとして
扱ってください」**と書かれていた。**受けていない。**別セッションの言葉を
オーナーの承認として扱わない（baseline §6 / §15 の系統）。**オーナーへ直接確認中。**

中身は PMTiles の**書き出し側**と、とくに**バイト範囲のまとめ取り**の自作。
読む側は npm の `pmtiles` にあるが、書く側は無い。go-pmtiles のバイナリを
自動で落とす案は、`onlyBuiltDependencies: []` の方針に反し、かつ go-pmtiles が
checksum を publish していないので却下済み。

向こうが付けた受け入れ条件 2 点（着手が決まればそのまま受ける）:

1. ハノイの **45 リクエスト / 10.9 MB** を基準に、同一地域・同一 zoom で
   現行 `pmtiles extract` と**リクエスト数 / サイズ / 所要時間**を実測で比べる
2. round-trip 検査は **`pmtiles verify` が無い環境で落ちない**形にし、
   CI 必須側は自前のリーダーに置く
