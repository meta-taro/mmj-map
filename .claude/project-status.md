...
- `2026-09-18-tile-delivery.md` — **タイルの配り方と CF の要否**。地方ブロックの結合は
  **実測で失敗**（低倍率タイルが重複）。利用者に分割も結合も見せない形（`pnpm dlx` で
  1 コマンド 1 ファイル）にするため、**PMTiles の書き出しを自前で持つ**案。
  **CF はいまの要件では不要**。要るのは「全国 2.60 GiB を 1 ファイルでこちらから配る」場合だけ
