...
- **現在フェーズ**: **S1 完了 / S2 完了**（2026-09-17）。S3 の入口（1 枚目をダークに）も決定済み。S2（UI 部品）は
...
1. ~~撮った地図を人が見て、良し悪しを言う~~ — **2026-09-17 に完了。**
   ...
   **S1 はこれで閉じました。**
2. **デザインの色を `DESIGN.md` に書く。****AI は `DESIGN.md` に手を入れません**（§11）。
   - **1 枚目がダークであることは決まりました**（D-016・2026-09-17）
   - ただし**色そのものはまだ `DESIGN.md` に無い**。いまスタイルに入っている値は
     **実装された値**であって、承認された配色ではない
   - `.claude/proposals/2026-09-16-design-umeda-namba.md` の色は**実装と別物**
     （提案 `#12151C` / 実物 `#14161A` など）。**提案は地図が 1 枚も描けていない時点で
     書かれたもの**で、本人が「実物を見た瞬間に覆りえる」と書いている
   - **実物の値を `DESIGN.md` へ写す**のが現実的だが、**書き込みには明示の許可が要る**
