...
2. product-baseline.md の「このリポの特例」     ← ここだけが上書きする
3. .claude/decisions.md（D-001〜）             ← この製品の設計判断
```

**特例は列挙されたものだけです**（`product-baseline.md` 冒頭）。
列挙に無いものは ECC に従います。**「project 側が全部勝つ」ではありません。**

## 必読

- **`~/.claude/rules/ecc/common/`** — 全製品共通の土台。**まずこれに従う**。
- **`.claude/rules/product-baseline.md`** — このリポの特例と、その理由。
