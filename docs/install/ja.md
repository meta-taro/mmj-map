...
> **まだ npm に出していません。**パッケージはこのリポジトリにありますが、全部 `private: true` で、
> `@mmj-map` スコープも取得していません。
> **`pnpm add @mmj-map/elements` は今日の時点では動きません。**
> 以下は**いま実際に動く手順**です。3 つをサイトへ置くだけです。
...
**ビルド工程はありません。**素の ES モジュールなので、束ねたくなければ束ねなくて構いません。
...
- **npm パッケージ。**未公開です。いまはファイルをコピーしてください
